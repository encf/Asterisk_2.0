...
void benchmark(const bpo::variables_map& opts) {
  ...
  for (size_t r = 0; r < repeat; ++r) {
    ...
    network->sync();
    OnlineEvaluator eval(nP, pid, network, std::move(preproc), circ, security_param, threads, seed);
...
bpo::options_description programOptions() {
  bpo::options_description desc("Asterisk legacy comparison online benchmark options");
  desc.add_options()
      ...
      ("x-clear", bpo::value<int64_t>()->default_value(-123), "Signed clear input x.")
...
int main(int argc, char* argv[]) {
  ...
  bpo::options_description cmdline("Benchmark legacy Asterisk comparison online phase.");
